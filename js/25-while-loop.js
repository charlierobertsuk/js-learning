...
if (targetNumber > 0 && targetNumber < 7) {
  // if not between 1 and 6 thin it is out of bounds
  while (dice !== targetNumber) {
    // when it has not rolled the target number, roll again
...
if (targetNumber > 1 && targetNumber < 13) {
  // if not between 1 and 6 thin it is out of bounds
  while (dice1 + dice2 !== targetNumber) {
    // when it has not rolled the target number, roll again
    console.log(
      `${rollCount + 1} - You rolled a ${
        dice1 + dice2
      } with a ${dice1} and a ${dice2}`
    );
    rollCount++;
    ...
    if (dice1 + dice2 === targetNumber) {
      console.log(
        `${
          rollCount + 1
        } - You rolled a ${targetNumber} with a ${dice1} and ${dice2}`
      );
      console.log(`It only took ${rollCount + 1} rolls!`);
    }
    if (rollCount === 100) {
